...
export default class GeoTimeline extends Component {
	...
	renderMapOrForm = () => {
		...
		if (this.state.patientId === '') {
			...
			return <MapContainer patientId={this.state.patientId} apiKey={apiKey} initialLat={this.state.initialLat} initialLon={this.state.initialLon}/>
		}
	...
	handleBrowserGeo = () => {
 	 ...
 	 if (!navigator.geolocation) {
    	...
    	navigator.geolocation.getCurrentPosition((success, error) => {
      	if(error) { this.setState({initialCenter: {lat: 43.6532, initialCenterLon: -79.3832}})}
      	else  {
        	console.log(success.coords.latitude)
        	this.setState({initialLat: success.coords.latitude, initialLon: success.coords.longitude})
      	}
