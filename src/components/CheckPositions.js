...
export class CheckPositions extends Component {
    ...
    getBoundaries() {
        let minLat = 1000.0, maxLat = -1000.0;
        let minLng = 1000.0, maxLng = -1000.0;
    ...
    loadConfirmedCases(boundaries) {
        const url = `${global.API_URL}/getfootprints?lng=${boundaries.lng[0]}~${boundaries.lng[1]}`
        ...
        for(let casePos of allCases) {
            ...
            for(let userPos of this.props.positions) {
                ...
                if(timeDiff > MAX_TIME_DIFF)
                    continue;
                // distance in meters
                const distance = Math.round(this.distance(casePos.latitude, casePos.longitude, userPos.lng, userPos.lat));
